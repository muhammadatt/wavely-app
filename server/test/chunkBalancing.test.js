...
test('falls back to plain target when no balanced count fits the duration rails', () => {
  // 360 s with minChunk 60 s → at most 6 chunks. A hint of 7 cannot be honoured
  // (one full wave would bust the minimum), so balancing returns null and the
  // planner uses the raw 120 s target → the same 3 chunks as the baseline.
  const p = plan(360, { concurrencyHint: 7 })
  ...
  for (const durationS of [200, 300, 360, 480, 600, 900]) {
    for (const concurrency of [2, 3, 4]) {
      ...
      for (const c of p.chunks) {
        ...
        assert.ok(durS >= 60 - 1 && durS <= 600 + 1,
          `dur=${durationS}s c=${concurrency}: chunk of ${durS.toFixed(1)}s out of [60,600] rails`)
      }
    }
  }
})
