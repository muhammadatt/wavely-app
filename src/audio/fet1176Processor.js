 ...
 *
 * ⚠⚠ THE APPLY PATH NO LONGER CALLS THIS — `applyFET1176Region` asks for the
 * flat `FET1176_PREROLL_S`, because the release tail this was built to outrun
 * was measured away: `TAIL_FRACTION` and `ALL_TAIL_FRACTION` both went to 0
 * with the FETish release fit, and the plugin converges (bit-exactly, in
 * all-buttons) at 2 s. The measurements below were taken BEFORE that fit and
 * describe the old tail; they are kept because the vocal chain's dynamics
 * section still reads this function for its own composite pre-roll, where the
 * FET is the slowest embedded envelope.
 *
 * ⚠ IT CAN ASK FOR UP TO 17.5 s (dial 1, all buttons in) AND IS DELIBERATELY
...
if (typeof registerProcessor === 'function') {
   ...
   * worklet, which composes FET1176Kernel. Load both into one AudioContext and the
