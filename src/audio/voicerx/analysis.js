 ...
 *
 * @returns {{ scanFreqs, scanEnv, baseline, flatBaseline }|null}
 */
export function estimateBaseline(
  freqsHz, envelopeDb, scanLowHz, scanHighHz, floorDb = -Infinity,
) {
  ...
  for (let k = 0; k < freqsHz.length; k++) {
    const f = freqsHz[k]
    if (f >= ctxLoLow && f < scanLowHz) lowVals.push(envelopeDb[k])
    else if (f > scanHighHz && f <= ctxHiHigh) highVals.push(envelopeDb[k])
    if (f >= scanLowHz && f <= scanHighHz) scanIdx.push(k)
  }
  if (lowVals.length === 0 || highVals.length === 0 || scanIdx.length < 2) return null

  let anchorLow = median(lowVals)
  let anchorHigh = median(highVals)

  const lowDead = anchorLow < floorDb
  const highDead = anchorHigh < floorDb
  ...
  return { scanFreqs, scanEnv, baseline, flatBaseline: lowDead || highDead }
}
 ...
 */
export function detectAnomaly(scanFreqs, scanEnv, baseline, thresholdDb, direction) {
  ...
  if (peak < thresholdDb) return result
...
export function scanRegions(freqsHz, envelopeDb, regions, envelopePeakDb) {
  ...
  for (const name of REGION_ORDER) {
    ...
    const det = detectAnomaly(
      ...
      region[THRESHOLD_DB], region[DIRECTION],
    )
