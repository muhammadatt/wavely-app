...
/**
 * Pre-Gain (input alignment), folded away on the faceplate and opened in
 * place.
 *
 * Collapsed, it is one engraved line — `PRE-GAIN −3.6 dB` — so the face stays
 * clean for the users who never need it. Clicking the line opens the LCD
 * stepper and AUTO lamp in the same slot; the ✕ (or Escape) folds it back.
 * The slot keeps one height in both states, so nothing on the face moves.
 *
 * ⚠ THE COLLAPSED LINE IS NOT DECORATION. The offset is a property of the
 * FILE, and a user who cannot see it fixes a mis-measured file with Input
 * instead — right for this file, and then carried into every preset they save.
 * So the number stays visible, and a hand-set value is loud about being one:
 * amber, and labelled MANUAL, because a forgotten override is the real cost of
 * folding the control away.
 *
 * Strictly it offsets the detector, not the audio; "Pre-Gain" is still the
 * right name, since that is what pre-gain into a compressor does to the
 * compression, and with Auto Makeup on it is what the user hears.
 */
...
.pg-readout:hover { color: #c9c5be; background: rgba(255,255,255,.05); }
.pg-readout:focus-visible { box-shadow: 0 0 0 1.5px rgba(255,164,53,.55); }
.pg-readout.is-manual { color: #d9a060; text-shadow: 0 0 4px rgba(255,140,40,.25); }
.pg-readout.is-manual:hover { color: #f0b876; }
...
.pg-close {
  ...
  background: transparent; color: #8f8b84; transition: background-color .15s ease, color .15s ease;
}
.pg-close:hover { background: rgba(255,255,255,.08); color: #f2f0ec; }
...
.pg-title {
  ...
  color: #f2f0ec; text-shadow: 0 1px 0 rgba(0,0,0,.9); white-space: nowrap;
}
