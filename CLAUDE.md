...
- **⚠ THE RUMBLE HEURISTIC HAS TWO STRUCTURAL DEFECTS, found on the first real recordings and NOT yet fixed** (documented at `CLEAN_TILT_DB` in `rumble.js`). **(1) It returns null — no analysis at all — on low corners.** The tilt bands are fractions of the corner and only a few FFT bins wide at `FRAME_SIZE` 2048, and below two bins the whole function bails silently. Swept 40–100 Hz: at **48 kHz every corner below 75 fails**; at 44.1 kHz every corner below 65 fails, and 75 fails while 70 and 80 pass — bin-alignment luck. A clean synthetic voice at F0 90 returns null outright. **Deep-voiced narrators get no rumble analysis and nothing says so** — the same too-few-bins failure this heuristic was written to replace in `sub_bass`. **(2) The clean tilt is not a constant.** On clean synthetic voices the shipped build measures tilt −15.76 / −10.22 / −8.24 at F0 120 / 180 / 220, i.e. gains of **0 / −3.78 / −5.76 dB on audio with nothing wrong** — `CLEAN_TILT_DB = −14` was calibrated at F0 120 and is really a function of where the corner lands.
- **The measurement is leakage-limited, and that is the first thing to fix.** At 48 kHz `FRAME_SIZE` 2048 gives 23.4 Hz resolution and a ~94 Hz Hann mainlobe — the entire 20–100 Hz region fits inside one mainlobe, and the HIGH band, nearest F0, catches the most leakage, so tilts read steeper than they are. Re-measured at frames 2048 / 8192 / 32768 on three real narrators: **−3.28 → +4.47 → +5.00** (48 kHz — sign flips, an 8.3 dB error), −3.43 → −3.87 → −3.90 (44.1 kHz), −9.57 → −8.23 → −8.35 (22 kHz). A 16384-sample window fixes defect 1 outright and is **cheaper** than what runs now. **Do not re-derive `CLEAN_TILT_DB` before fixing the window** — it would be fitted to leakage.
- **On real audio the heuristic's ordering looks right, which is why this is a fix and not a removal.** It offers −10.72 / −10.57 / −4.43 dB on three real files whose 40–60 Hz bands sit at −7.9 / −11.5 / −16.2 dB relative to their own 300 Hz–3 kHz speech band — most cut where there is most LF energy, least on the hard-mastered file that was already high-passed. ⚠ Still **no known-clean unmastered recording**; the mastered file cannot serve as the clean reference without assuming what it is meant to prove.
