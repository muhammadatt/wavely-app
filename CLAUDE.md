...
- Markers (`src/audio/markers.js`) — point markers only; a region/slice is the span between two adjacent markers plus the head and tail, derived by `markerSpans` and never stored, so there is one representation of "what is a slice". Drawn as flags in the waveform's ruler gutter and as ticks on the overview; **the gutter is the marker lane and the body is the selection lane**, split by y, which is why markers could be added without touching the selection gesture at all. A marker is a timeline position, so the `markersAfter*` transforms run one-for-one against the length-changing mutations in `operations.js` — the ones that preserve length need none, and that list is longer than it looks (`silenceRegion` swaps in an equal-duration SilenceSegment, splits only insert a boundary, and `replaceRegionWithBuffer` pins `sourceEnd = end - start` so an effect render cannot move a marker either). ⚠ FOUR PLACES PUSH UNDO HISTORY, not one — `pushUndo`, the pushes inside `undo` and `redo`, and `replaceRegion`'s own inline copy — and all four carry markers alongside segments. Placement funnels through one `placeMarkerTime` so the zero-crossing snap (`src/audio/zeroCross.js`, rising crossings only, on by default) cannot be honoured at one entry point and missed at another. Gap markers mark the span they open as dead air; "Split at markers" and "Delete gaps" both act on `markerSpans`. Audacity label TSV in and out — ⚠ import must NOT use `normalizeMarkers`, which keeps the first of a colliding pair and so drops every name in a contiguous file. **Splitting is in place only; rendering each slice to its own file is not built.**
...
- **Test infrastructure** — Partial. `npm test` runs a `node:test` unit suite over the client DSP (`test/dsp/`, `test/voicerx/`, `test/ui/`, 1062 tests). No integration or E2E tests, and no coverage of the server pipeline, Vue components or the async job flow — `test/ui/` reaches the editor state composable and the playback scheduler by faking the AudioContext, which is as far up as this suite goes. **⚠ Two seams are known blind spots and both shipped broken through a green suite in one day:** the measurement worker's reply contract (now driven against a fake `self` in `test/ui/processWorkerContract.test.js`), and the composable→panel destructure — nothing type-checks it, so a composable that stops exporting a function still builds and still passes, and the panel is simply dead on open. Diff a composable's exported keys against its modal's destructure after editing either. **`npm run smoke` is the only instrument that sees the module-scope form of this** (a re-export without a matching local import, an undeclared identifier) — it opens every panel in a browser and fails on an uncaught exception. It has now caught what shipped three times; run it before merging anything that touches a composable, an effect wrapper or a params module
- **Slice export** — Markers can split the timeline in place but cannot yet render each slice to its own file. Every piece exists: `markerSpans` → `renderRegionToBuffer` (`src/audio/processing.js`) → the WAV encoder in `src/audio/export.js` → `createZip` (`src/audio/zip.js`); only the wiring and a naming scheme are missing.
- **Silence detection** — Nothing in the codebase detects silence. `RemoveSilenceWindow.vue` is a UI stub whose Apply shows "coming soon", and auto-markers / smart-split need the same detector. One frame-RMS walk over the timeline (in the manner of `readTimelineEnvelope`) would feed all three.
