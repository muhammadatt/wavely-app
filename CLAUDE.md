...
- **THE OPTO'S PEAK REDUCTION TAPER IS ~2.4x TOO AGGRESSIVE, measured against a reference emulation and not yet fixed.** On the same clip, a third-party LA-2A emulation at Peak Reduction 75 applied **3.97 dB** of average reduction; ours at 75 applies **9.43 dB**. Matching their 75 needs our knob at **48.7**. `SC_DRIVE_SPAN_DB = 40` with `SC_TAPER = 0.7` is putting far too much drive on the upper half of the travel.
  - **This, not the ballistics, is what made the compressor look broken.** Crest factor rises with depth — 20.68 → 20.76 at 9.4 dB of GR, → 22.29 at 12.7 — so at ordinary-looking knob positions users land in a regime where the unit reduces the average more than the peaks, and auto-makeup then lifted the survivors. **At MATCHED gain reduction our transient control is better than the reference's**: crest change −1.95 dB against their −1.22, peak reduction 5.92 against 5.20. The 10 ms attack is fine and stays.
  - **Do not recalibrate from one operating point.** Fixing the taper needs the same clip through the reference at several knob positions; one measurement moves one point, it does not fit a curve.
- **Scheps Parallel** — Andrew Scheps' Pultec/LA-2A vocal trick as one plugin. Pultec push → OptoSmooth → Pultec recovery on a wet path, blended against the dry. **Four visible controls** (Character, Squash, Mix, Output) because the trick's defining settings are not choices: the LA-2A's R37 emphasis is pinned wide open (that *is* the trick — the side-chain filtered so the cell stops ducking on plosives and rides the presence band instead), mode is COMP, and its makeup stays at 0 so the tube stage isn't driven by gain that belongs after the post EQ.
  ...
  - **Squash defaults to 80, not a mid position, because the side-chain arrives twice-filtered.** The pre EQ takes ~4.7 dB out of the lows and R37 takes 10 dB more below 1 kHz, so the cell sees far less than the raw signal: 80 here lands around 7 dB of gain reduction on speech at nominal level, where the same number on the Opto Comp panel with a flat side-chain gives about 13.
...
- **Test infrastructure** — Partial. `npm test` runs a `node:test` unit suite over the client DSP (`test/dsp/`, `test/voicerx/`, 310 tests). No integration or E2E tests, and no coverage of the server pipeline, Vue components or the async job flow
