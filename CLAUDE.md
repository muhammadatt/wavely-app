...
- **Scheps Parallel** — Andrew Scheps' Pultec/LA-2A vocal trick as one plugin. Pultec push → OptoSmooth → Pultec recovery on a wet path, blended against the dry. **Four visible controls** (Character, Squash, Mix, Output) because the trick's defining settings are not choices: the LA-2A's R37 emphasis is pinned wide open (that *is* the trick — the HF knob all the way up so the cell stops ducking on plosives and rides the presence band instead), mode is COMP, and its makeup stays at 0 so the tube stage isn't driven by gain that belongs after the post EQ.
  ...
  - **Auto Output Trim is measured per region, in the worker**, alongside the correlation, by the same debounce/supersede machinery the compressors' auto-makeup uses. It sets the wet path's gain *before* the sum, so pushing Mix changes character rather than loudness — without it the Mix knob is a volume control and every A/B is decided by level. Confirmed live in the app: IN −11.9 dB, OUT −12.2 dB at the default Mix with 6.5 dB of gain reduction on the wet path.
...
- **Test infrastructure** — Partial. `npm test` runs a `node:test` unit suite over the client DSP (`test/dsp/`, `test/voicerx/`, 302 tests). No integration or E2E tests, and no coverage of the server pipeline, Vue components or the async job flow
