...
- **VoiceRx is measured, and the numbers are poor.** `npm run scorecard` runs a 58-case synthetic corpus (`test/voicerx/`) in which a clean voice is corrupted with a *known* EQ move, so the correct output is the exact inverse and needs no human judgement. The current detector's baseline (`test/voicerx/baseline.json`): **detection 67%**, **recovery median 0.50** with a **spread of 0.58** (p10 0.16 / p90 0.74), **27 invented bands totalling 90.1 dB**, and **5 of 16 cases needing no correction get one**. Aim is the one strong result — median 0.021 octaves off. Blind spots are structural: `body_warmth`, `lower_presence` and `air` are dip-only with `MAX_CUT_DB = 0`, so a hump anywhere in 1200–2500 Hz (or at 180 Hz, or 11 kHz) is **undetectable by construction**, and deficiency detection overall is 29%. `test/voicerx/scorecard.test.js` pins a 6-case subset as a regression guard — it asserts the numbers are *unchanged*, not that they are good; re-record with `node scripts/voicerx-scorecard.mjs --smoke --write-baseline`.
...
- **v2's top-end failure did not transfer either.** It scored zero bandwidth-spurious bands on synthetics; on real audio it emits a **12-band cluster at 11.3 kHz** plus 2 at 16 kHz, and reports suppressed "resonances" of **19.8–23.4 dB at 11.3–15.9 kHz** on four windows — the roll-off read as a giant peak, landing in advisories instead of bands only because the plausibility term caught it. Add this to the list of things the synthetic corpus cannot see.
...
- **Three ground-truth-free checks (`npm run robustness`, `npm run robustness:real`).** Invariance (gain and polarity must change nothing — anything under 100% is a bug; window shifts held to a lower bar), convergence (apply a detector's own corrections, re-analyse — a detector reading the voice has little left to say), and direction (boosts above 3 kHz, where a lift adds the very defects users come to remove). They work on any audio, annotated or not. **All three detectors pass everything on the synthetic corpus** — which is a finding about the corpus, not the detectors: it cannot exercise these either. **On the 13-file real corpus they separate, and one of them found an actual bug:**
  ...
  | on finished masters | v1 | `v1trend` | v2 |
  |---|---|---|---|
  | exact invariance (min) | **100%** | **100%** | **89% — a bug** |
...
- **The v2 invariance bug is not the absolute epsilon, and is still unattributed.** Both detectors floor their log-power with an absolute `+ 1e-10` (`analysis.js:430`, `envelope.js:202`) and v2 alone runs the cepstrum on *pause* frames, which made it the obvious suspect. It is not: attenuating a synthetic case to −60 dBFS, and separately gating its pauses to −40/−80/−140 dB, leaves **all three detectors at 100%**. So the synthetic corpus cannot reproduce this either (it has no gated pauses — 13 of 31 real windows do, and zero synthetic ones), and the mechanism is unknown. `invariance()` now records which band moved under which transformation, so the next real run names it instead of reporting a percentage nobody without the file can chase. **No shipping code was changed on the strength of the guess.**
- **Repeatability needs the band-count confound removed before it can be read.** A per-file fraction charges a quiet detector more for the same absolute mistake — one stray band out of two is 50%, one out of five is 80% — which is exactly the shape of v1 (2.87 bands/window, 75%) against `v1trend` (1.55, 56%). `scorecard:real` now also reports a **pooled** fraction (sum before dividing) and **absolute bands that failed to recur, per file**, which no denominator effect can distort. Until those two are read, the 75%/56% gap is not evidence that `v1trend` is less stable.
