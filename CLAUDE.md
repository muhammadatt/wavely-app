...
- **v2's open problems (do not ship it yet):** on the real reference clip it emits **10 bands where v1 emits 2**, several of them alternating ±2 dB corrections about 1/6 octave apart — chasing envelope ripple rather than diagnosing, which the synthetic corpus cannot see because its residuals are smooth. Needs a feature-count policy and/or a prominence/spacing rule. Also: a defect centred on the spectrum's **own maximum** is near-invisible to any smooth reference (a planted +12 dB peak at 300 Hz leaves only 2.25 dB of residual at the chosen span) — measured, inherent, not tunable. Recovery spread is better but still 0.38.
