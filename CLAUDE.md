...
- **Test infrastructure** — Partial. `npm test` runs a `node:test` unit suite over the client DSP (`test/dsp/`, `test/voicerx/`, `test/ui/`, 339 tests). No integration or E2E tests, and no coverage of the server pipeline, Vue components or the async job flow — `test/ui/` reaches the editor state composable and the playback scheduler by faking the AudioContext, which is as far up as this suite goes
