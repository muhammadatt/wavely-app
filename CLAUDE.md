...
- **Three ground-truth-free checks (`npm run robustness`, `npm run robustness:real`).** Invariance (gain and polarity must change nothing — anything under 100% is a bug; window shifts held to a lower bar), convergence (apply a detector's own corrections, re-analyse — a detector reading the voice has little left to say), and direction (boosts above 3 kHz, where a lift adds the very defects users come to remove). They work on any audio, annotated or not. **All three detectors pass everything on the synthetic corpus** — which is a finding about the corpus, not the detectors: it cannot exercise these either. On a **stand-in** real corpus the picture separates immediately — v2 drops to 86% framing invariance where both v1 variants hold at 100% — but that has **not yet been reproduced on the 13-file corpus**, whose robustness run silently executed the synthetic set instead (npm ate the `--real` flag on PowerShell; hence the dedicated `robustness:real` script and the banner naming the corpus).
